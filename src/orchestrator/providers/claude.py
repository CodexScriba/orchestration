...
class ClaudeProvider(BaseProvider):
    """Invoke the Claude CLI using provider markdown frontmatter config."""
    ...
    def build_command(self, prompt: str | None = None) -> list[str]:
        """Build the Claude CLI command for execution.
        ...
        if prompt is not None:
            ...
            # stdin style: prompt delivered via stdin, not appended here
    ...
    def invoke(
        ...
        """Run a Claude invocation and return its execution result."""
        ...
        if prompt_style in ("flag", "positional"):
            # Prompt embedded in command; pass empty string to session stdin
